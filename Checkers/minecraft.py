...
class Main:
    def __init__(self):
        ...
        print(t)
        print(f'{yellow}[!] Feel free to edit Settings.yml to alter the way this checker operates\n')
    ...
    def loadcombo(self):
        while True:
            try:
                print(f"{cyan}Please Import Your Combos")
                sleep(0.3)
                loader = open(fileopenbox(title="Load Combos", default="*.txt"), 'r', encoding="utf8",
            ...
            try:
                if OxygenX.Proxy.proxy:
                    ...
                    if not OxygenX.Proxy.API.use:
                        print(f"\n{cyan}Please Import Your HTTP Proxies.....")
                        sleep(0.3)
                        loader = open(fileopenbox(title="Load Proxies", default="*.txt"), 'r', encoding="utf8",
                                      errors='ignore').read().split('\n')
...
if __name__ == '__main__':
    ...
    charz = ['@', '!', '#', '$', '%', '^', '&', '*', ')', '(', '-', '}', '{', ']', '"', '+', '=', '?', '/',
             '.', '>', ',', '<', '`', '\'', '~', '[', '\\', ' ']
    version = '0.8'
    set_title(f'Minecraft Checker | by ShadowOxygen')
    t = f'''{cyan} ▄▀▀▀▀▄  ▄▀▀▄ ▄▄   ▄▀▀▀▀▄   ▄▀▄▄▄▄   ▄▀▀▄ █ 
█ █   ▐ █  █   ▄▀ █      █ █ █    ▌ █  █ ▄▀ 
   ▀▄   ▐  █▄▄▄█  █      █ ▐ █      ▐  █▀▄  
▀▄   █     █   █  ▀▄    ▄▀   █        █   █ 
